...
export async function buildReport(wallet) {
  ...
  // Total APY: same rewards, but relative to ALL SOL held (staked + idle) over the holding span
  const holdSpan = grid.map((e, i) => ({ e, h: hold[i] })).filter((x) => x.h > 1e-6);
  ...
  const totalApy = avgHold > 0 && yearsHold > 0.01 ? Math.pow(1 + earned / avgHold, 1 / yearsHold) - 1 : 0;
